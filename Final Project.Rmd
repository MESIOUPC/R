...
Exploratory data analysis about Catalonia's seawater historical monthly average temperature at different depths (at 0, -20, -50, and -80 meters). 
...
```{r}
label(sea.deep) <- 
  "Catalonia's seawater monthly average temperature at different depths (2000-2019)"
...
label(sea.deep.historical) <- 
  "Catalonia's seawater historical monthly average temperature at different depths
(previous 30 years)"
...
plot(annual_var[,2] ~ annual_var[,1], 
     type = "o", col = "red", xlab = "Year", ylab = "Avg. Temperature  (C)", 
...
Timeline of average temperatures during the years.
...
for (i in 2:5) {
  ...
  theme(panel.grid.major.x = element_blank(),
        ...
        axis.ticks.x = element_blank(), text = element_text(family = "serif")) +
  xlab("Year")+
  ylab("Temperature (C)")
  )
}
...
Heatmap of yearly temperatures by depth.

```{r fig3b, fig.height = 4, fig.width = 9.5, fig.align = "center"}
...
ggplot(hm, aes(year, factor(depth), fill = temp)) + 
  ...
  scale_fill_gradient(low="#d0e8f2", high="#456268") + ylab('depth') +
  ggtitle('Average Temperatures Year Over Year') + 
...
Scatterplot showing correlation between surface temperature (0m) and temperature at -80m.

```{r fig3c, fig.height = 4, fig.width = 4, fig.align = "center"}

ggplot(sea.deep, aes(x = sea.deep$'0', y = sea.deep$'-80')) +
geom_point() + 
  geom_abline(aes(intercept = mean(sea.deep$'-80') - mean(sea.deep$'0'), slope = 1), 
              linetype = 2, color = "red") + 
  scale_fill_gradient(low="#d0e8f2", high="#456268") + xlab('0m') + ylab('-80m') +
  ggtitle('Correlation Between Depth Differences in Temperature') + 
  theme(text = element_text(family = "serif")) + 
...
Creation of simulated temperatures using normal truncated distribution.
...
for (i in c('01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12')) {
  sim[which(sea.deep$month == i), 4] <- 
    rtruncnorm(20, mean = mean(sea.deep[which(sea.deep$month == i), 4]), sd(sea.deep[which(sea.deep$month == i), 4]))
  sim[which(sea.deep$month == i), 5] <- 
    rtruncnorm(20, mean = mean(sea.deep[which(sea.deep$month == i), 5]), sd(sea.deep[which(sea.deep$month == i), 5]))
  sim[which(sea.deep$month == i), 6] <- 
    rtruncnorm(20, mean = mean(sea.deep[which(sea.deep$month == i), 6]), sd(sea.deep[which(sea.deep$month == i), 6]))
  sim[which(sea.deep$month == i), 7] <- 
    rtruncnorm(20, mean = mean(sea.deep[which(sea.deep$month == i), 7]), sd(sea.deep[which(sea.deep$month == i), 7]))

  sim.historical[which(sea.deep.historical$month == i), 4] <- 
    rtruncnorm(20, mean = mean(sea.deep.historical[which(sea.deep.historical$month == i), 4]), sd(sea.deep.historical[which(sea.deep.historical$month == i), 4]))
  sim.historical[which(sea.deep.historical$month == i), 5] <- 
    rtruncnorm(20, mean = mean(sea.deep.historical[which(sea.deep.historical$month == i), 5]), sd(sea.deep.historical[which(sea.deep.historical$month == i), 5]))
  sim.historical[which(sea.deep.historical$month == i), 6] <- 
    rtruncnorm(20, mean = mean(sea.deep.historical[which(sea.deep.historical$month == i), 6]), sd(sea.deep.historical[which(sea.deep.historical$month == i), 6]))
  sim.historical[which(sea.deep.historical$month == i), 7] <- 
    rtruncnorm(20, mean = mean(sea.deep.historical[which(sea.deep.historical$month == i), 7]), sd(sea.deep.historical[which(sea.deep.historical$month == i), 7]))
}


```

Function testing with the original data set:

```{r fig6, fig.height = 2, fig.width = 3.5, fig.align = "center"}
temp_mom(sea.deep)
temp_diff(sea.deep, sea.deep.historical)
```

Function testing with the simulated data set:

```{r fig7, fig.height = 2, fig.width = 3.5, fig.align = "center"}
temp_mom(sim)
