...
author: "Nuria Saavedra Miro nd Ferran Vilar Pages"
...
plot(annual_var[,2] ~ annual_var[,1], 
     ...
     main = "Annual Variation", ylim = c(13, 23))
...
legend(2000, 23, legend=c("0m", "-20m", "-50m", "-80m"),
...
```{r fig3a, fig.height = 3, fig.width = 6, fig.align = "center"}
...
```
```{r fig3b, fig.height = 2, fig.width = 9, fig.align = "center"}

# heatmap of depth / date (temp)
hm <- 
  union(
  union(
   cbind(sea.deep[, 1:3], depth = 0, temp = as.numeric(sea.deep$'0')),
   cbind(sea.deep[, 1:3], depth = -20, temp = as.numeric(sea.deep$'-20'))),
  union(
   cbind(sea.deep[, 1:3], depth = -50, temp = as.numeric(sea.deep$'-50')),
   cbind(sea.deep[, 1:3], depth = -80, temp = as.numeric(sea.deep$'-80')))
  )

ggplot(hm, aes(year, factor(depth), fill= temp)) + 
  geom_tile() + 
  scale_fill_gradient(low="#d0e8f2", high="#456268") + ylab('depth') + ggtitle('Temperatures over the years') + theme(text = element_text(family = "serif")) 

```

```{r fig3c, fig.height = 2, fig.width = 9, fig.align = "center"}

# correlation between temperatures? scatter plot??
